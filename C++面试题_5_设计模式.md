...
public:
    void processRequest(const Request& request) const override {
        if (request.getType() == Request::Type::LEAVE && request.getAmount() <= 3) {
            std::cout << "Director approved " << request.getAmount() << " days leave" << std::endl;
        } else if (successor) {
...
public:
    void processRequest(const Request& request) const override {
        if ((request.getType() == Request::Type::LEAVE && request.getAmount() > 3 && request.getAmount() <= 7) ||
            ...
            std::cout << "Manager approved request (amount: " << request.getAmount() << ")" << std::endl;
        } else if (successor) {
            successor->processRequest(request);
        }
    }
...
public:
    void processRequest(const Request& request) const override {
        if ((request.getType() == Request::Type::LEAVE && request.getAmount() > 7) ||
            ...
            std::cout << "CEO approved request (amount: " << request.getAmount() << ")" << std::endl;
        } else if (successor) {
            successor->processRequest(request);
        }
    }
...
protected:
    ...
    void addCondiments() override {
        std::cout << "Adding milk and sugar to coffee..." << std::endl;
    }
};
